	 ...
	 if(save_date.flash_or_define==USE_FLASH_DATE)//如果读取到标志位为flash使用flash中的数据
		...
		else//否则使用define中的数据
		{
		  ...
		  mid_date.Pitch_speed_pid[2]=PITCH_SPEED_PID_KD;		
			mid_date.Pitch_speed_pid[3]=PITCH_SPEED_PID_KF;			
      ...
      mid_date.YAW_GYRO_ABSOLUTE_PID[2]=YAW_GYRO_ABSOLUTE_PID_KD;
			...
			mid_date.Yaw_speed_pid[3]=YAW_SPEED_PID_KF;
...
static cJSON *ctr_mode(cJSON *dat)
{
	...
	else if(strcmp(cJSON_GetObjectItem(dat,"mode")->valuestring,visual_m)==0)
	{
	  	...
	  	gimbal_ctrl.flag_shoot = cJSON_GetArrayItem(date,2)->valuedouble;//传入接收到的数据到shoot参数	
			gimbal_ctrl.add_yaw_v=cJSON_GetArrayItem(date,3)->valuedouble;     //传入接收到的数据到yaw视觉参数
			gimbal_ctrl.add_yaw_a=cJSON_GetArrayItem(date,4)->valuedouble;     //传入接收到的数据到yaw视觉参数	  
